    ...
    def add_categorical_slots(self, state_update):
        ...
        for slot_idx, slot in enumerate(categorical_slots):
            ...
            if not values:
                ...
                unknown_nl_seq = " ".join(
                    [service_des, slot, slot_descriptions[slot], schema_constants.STR_UNKNOWN])
                unknown_paired_input_ids, unknown_paired_mask, unknown_paired_seg = self.create_context_schema_features(unknown_nl_seq)
                unknown_cat_slot_example = CatSlotExample(self.example_id, self.service_id, unknown_paired_input_ids, unknown_paired_mask, unknown_paired_seg, slot_idx, cat_slot_status, 0, schema_constants.STATUS_ACTIVE)
                cat_slot_examples.append(donotcare_cat_slot_example)
                cat_slot_examples.append(unknown_cat_slot_example)
...
# all slot value is off, but the slot value off is on
                ...
                unknown_nl_seq = " ".join(
                    [service_des, slot, slot_descriptions[slot], schema_constants.STR_UNKNOWN])
                unknown_paired_input_ids, unknown_paired_mask, unknown_paired_seg = self.create_context_schema_features(unknown_nl_seq)
                unknown_cat_slot_example = CatSlotExample(self.example_id, self.service_id, unknown_paired_input_ids, unknown_paired_mask, unknown_paired_seg, slot_idx, cat_slot_status, 0, schema_constants.STATUS_OFF)
                cat_slot_examples.append(donotcare_cat_slot_example)
                cat_slot_examples.append(unknown_cat_slot_example)
            else:
                ...
                unknown_nl_seq = " ".join(
                    [service_des, slot, slot_descriptions[slot], schema_constants.STR_UNKNOWN])
                unknown_paired_input_ids, unknown_paired_mask, unknown_paired_seg = self.create_context_schema_features(unknown_nl_seq)
                unknown_cat_slot_example = CatSlotExample(
                    self.example_id, self.service_id, unknown_paired_input_ids,
                    unknown_paired_mask, unknown_paired_seg, slot_idx, cat_slot_status, 0, schema_constants.STATUS_OFF)
                cat_slot_examples.append(donotcare_cat_slot_example)
                cat_slot_examples.append(unknown_cat_slot_example)
