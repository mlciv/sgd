...
class DSTC8BaselineTopTransModel(PreTrainedModel):
    """
    ...
    def __init__(self, config=None, args=None):
        super(DSTC8BaselineTopTransModel, self).__init__(config=config)
    ...
    def create_or_load_schema_embedding(self, args, split):
        ...
        for service in schema_data:
            ...
            schema_data_dict["cat_slot_mask"].append(service["cat_slot_mask"])
            schema_data_dict["cat_slot_value_emb"].append(service["cat_slot_value_emb"])
            schema_data_dict["cat_slot_value_mask"].append(service["cat_slot_value_mask"])
            schema_data_dict["noncat_slot_emb"].append(service["noncat_slot_emb"])
            schema_data_dict["noncat_slot_mask"].append(service["noncat_slot_mask"])
            schema_data_dict["req_slot_emb"].append(service["req_slot_emb"])
            schema_data_dict["req_slot_mask"].append(service["req_slot_mask"])
            schema_data_dict["intent_emb"].append(service["intent_emb"])
            schema_data_dict["intent_mask"].append(service["intent_mask"])

        schema_tensors = {}
        for key, array in schema_data_dict.items():
            if key.endswith("mask"):
                schema_tensors[key] = torch.tensor(np.asarray(array, np.int32)).long().cpu()
            else:
    ...
    def _get_intents(self, features, _encoded_tokens):
    ...
    def _get_requested_slots(self, features, _encoded_tokens):
        """Obtain logits for requested slots."""
        slot_embeddings = features["req_slot_emb"].index_select(0, features["service_id"])
        slot_mask = features["req_slot_mask"].index_select(0, features["service_id"])
        utterance_mask = features["utt_mask"]
        logits, _ = self._get_logits(
    ...
    def _get_categorical_slots_goals(self, features, _encoded_tokens):
        ...
        slot_mask = features["cat_slot_mask"].index_select(0, features["service_id"])
        utterance_mask = features["utt_mask"]
        ...
        value_mask = features["cat_slot_value_mask"].index_select(0, features["service_id"])
    ...
    def _get_noncategorical_slots_goals(self, features, _encoded_tokens):
        ...
        slot_mask = features["noncat_slot_mask"].index_select(0, features["service_id"])
        utterance_mask = features["utt_mask"]
    ...
    def forward(self, features, labels=None):
        ...
        outputs = {}
        outputs["logit_intent_status"] = self._get_intents(features, _encoded_tokens)
        outputs["logit_req_slot_status"] = self._get_requested_slots(features, _encoded_tokens)
        cat_slot_status, cat_slot_value = self._get_categorical_slots_goals(features, _encoded_tokens)
        ...
        noncat_slot_status, noncat_span_start, noncat_span_end = (
            self._get_noncategorical_slots_goals(features, _encoded_tokens))
...
class SchemaInputFeatures(object):
    ...
    def __init__(self, input_ids, input_mask, input_type_ids,
                 embedding_tensor_name, mask_tensor_name,
                 service_id, intent_or_slot_id, value_id):
...
class SchemaEmbeddingGenerator(nn.Module):
    ...
    def _create_feature(self, input_line, embedding_tensor_name,
                        mask_tensor_name, service_id,
        ...
        input_ids = batch_encodings["input_ids"][0,:].cpu().numpy()
        input_mask = batch_encodings["attention_mask"][0,:].cpu().numpy()
        input_type_ids = batch_encodings["token_type_ids"][0,:].cpu().numpy()
        ...
        input_mask = torch.from_numpy(input_mask).view(1, -1).long()
        input_type_ids = torch.from_numpy(input_type_ids).view(1, -1).long()
    ...
    def _get_goal_slots_and_values_input_features(self, service_schema):
        ...
        for slot_id, slot in enumerate(service_schema.non_categorical_slots):
            nl_seq = " ".join(
                [service_des, _NL_SEPARATOR, slot, slot_descriptions[slot]])
            features.append(self._create_feature(nl_seq, "noncat_slot_emb", "noncat_slot_mask",
                                                 service_schema.service_id, slot_id))

        for slot_id, slot in enumerate(service_schema.categorical_slots):
            nl_seq = " ".join(
                [service_des, _NL_SEPARATOR, slot, slot_descriptions[slot]])
            features.append(self._create_feature(nl_seq, "cat_slot_emb", "cat_slot_mask",
                                                 service_schema.service_id, slot_id))
            for value_id, value in enumerate(
                ...
                features.append(self._create_feature(
                    nl_seq, "cat_slot_value_emb", "cat_slot_value_mask", service_schema.service_id, slot_id,
                    value_id))
        return features
