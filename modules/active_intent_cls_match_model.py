...
# --------------------------------------------------------------------
# File Name          : active_intent_cls_match_model.py
...
class ActiveIntentCLSMatchModel(PreTrainedModel, DSTC8BaselineOutputInterface):
    """
    ...
    def _encode_schema(self, features, schema_type, is_training):
        ...
        if self.config.schema_finetuning_type == "finetuning_cls":
            ...
            encoded_schema_cls = features[SchemaInputFeatures.get_embedding_tensor_name("intent")]
            # Apply dropout in training mode.
            if is_training:
                encoded_schema_cls = self.utterance_dropout(encoded_schema_cls)
    ...
    def _get_intents(self, features, _encoded_utterance, is_training):
        """Obtain logits for intents."""
        # we only use cls token for matching, either finetuned cls and fixed cls
        intent_embeddings, _ = self._encode_schema(features, "intent", is_training)
    ...
    def forward(self, features, labels=None):
        ...
        outputs = {}
        outputs["logit_intent_status"] = self._get_intents(features, _encoded_utterance, is_training)
