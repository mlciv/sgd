...
# --------------------------------------------------------------------
# File Name          : flat_requested_slots_dual_encoder.py
...
class FlatRequestedSlotsDualEncoder(PreTrainedModel, EncodeSepUttSchemaInterface, FixedSchemaCacheEncoder, FlatDSTOutputInterface):
    """
    ...
    def __init__(self, config=None, args=None, encoder=None, utt_encoder=None, schema_encoder=None):
        super(FlatRequestedSlotsDualEncoder, self).__init__(config=config)
        ...
        if self.config.matcher == "ConcatenateMatcher":
            self.requested_slot_matching_layer = ConcatenateMatcher(encoding_dim=self.utterance_embedding_dim)
        elif self.config.matcher == "DotProductMatcher":
            self.requested_slot_matching_layer = DotProductMatcher(encoding_dim=self.utterance_embedding_dim)
        elif self.config.matcher == "BilinearMatcher":
            self.requested_slot_matching_layer = BilinearMatcher(encoding_dim=self.utterance_embedding_dim)
        else:
            raise NotImplementedError("Not implemented for matcher {}".format(self.config.matcher))

        self.requested_slot_final_dropout = torch.nn.Sequential()
        # Project the combined embeddings to obtain logits.
        # then max p_active, if all p_active < 0.5, then it is null
        if self.requested_slot_matching_layer.outputDim == 1:
            self.requested_slot_final_proj = torch.nn.Sequential(
            )
        else:
            self.requested_slot_final_proj = torch.nn.Sequential(
                torch.nn.Linear(self.requested_slot_matching_layer.outputDim, 1)
            )
    ...
    def _get_requested_slots(self, features, is_training):
        ...
        encoded_schema_cls, encoded_schema_tokens, encoded_schema_mask = self._encode_schema(
            self.tokenizer, self.schema_encoder, features, self.schema_dropout, self.scalar_schema_mix, self.req_slot_seq2_key, is_training)
        logits = self._get_logits(
            encoded_schema_cls,
            encoded_utt_cls,
            self.requested_slot_matching_layer, self.requested_slot_final_proj, is_training)
        # Shape: (batch_size, 1)
