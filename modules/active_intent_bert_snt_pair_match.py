...
# --------------------------------------------------------------------
# File Name          : active_intent_bert_snt_pair_match_model.py
...
class ActiveIntentBERTSntPairMatchModel(PreTrainedModel, EncodeUttSchemaPairInterface, DSTC8BaselineOutputInterface):
    """
    ...
    def __init__(self, config=None, args=None, encoder=None):
        super(ActiveIntentBERTSntPairMatchModel, self).__init__(config=config)
    ...
    def _get_intents(self, features, is_training):
        ...
        max_num_intents = logits.size()[-1]
        # [batch_size, max_intent+1]
        mask = torch_ext.sequence_mask(
            features["intent_num"],
            maxlen=max_num_intents, device=self.device, dtype=torch.bool)
    ...
    def define_loss(cls, features, labels, outputs):
        ...
        if "logit_intent_status" in outputs:
            ...
            # Shape: (batch_size, max_num_intents).
            intent_labels = labels["intent_status"].type_as(intent_logits)
            # (batch_size, max_intent_num )
            intent_weights = torch_ext.sequence_mask(
                features["intent_num"],
                maxlen=max_intent_num, device=intent_labels.device, dtype=torch.float)
