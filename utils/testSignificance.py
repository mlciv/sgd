...
#Permutation-randomization for torch version
#Repeat R times: randomly flip each m_i(A),m_i(B) between A and B with probability 0.5, calculate delta(A,B).
# let r be the number of times that delta(A,B)<orig_delta(A,B)
# significance level: (r+1)/(R+1)
# Assume that larger value (metric) is better
def rand_permutation_torch(data_A, data_B, n, R):
    with torch.no_grad():
        tensorA = torch.tensor(data_A).cuda()
        tensorB = torch.tensor(data_B).cuda()
        batch_size = 100
        tensorAB = tensorA - tensorB
        delta_orig = tensorAB.mean()
        dup_tensorAB = tensorAB.unsqueeze(0).expand(batch_size, n)
        dup_tensorBA = - dup_tensorAB
        r = 0
        for x in tqdm(range(0, R, batch_size)):
            mask = torch.randint(0, 2, (batch_size, n), dtype=torch.bool, device="cuda") # 1 for keep, 0 for exchange
            # batch_size x n -> batch_size, we just swap the sign of the delta
            delta = torch.where(mask, dup_tensorAB, dup_tensorBA).mean(dim=1)
            batch_count = (delta >= delta_orig).sum()
            r = r + batch_count
        pval = float(r+1.0)/(R+1.0)
    return pval
...
# This implementation follows the description in Berg-Kirkpatrick et al. (2012), 
# "An Empirical Investigation of Statistical Significance in NLP".
def Bootstrap_torch(data_A, data_B, n, R):
    with torch.no_grad():
        tensorA = torch.tensor(data_A).cuda()
        tensorB = torch.tensor(data_B).cuda()
        batch_size = 100
        ...
        for x in tqdm(range(0, R, batch_size)):
            ...
            sample_index = torch.randint(0,n,(batch_size, n), dtype=torch.long, device="cuda")
            sample_delta = dup_tensorAB.gather(1, sample_index).mean(dim=1)
            batch_count = (sample_delta >= 2*delta_orig).sum()
            r = r + batch_count
        pval = float(r)/(R)
    return pval
...
def main():
    ...
    if(name=="Permutation"):
        ...
        pval = rand_permutation_torch(data_A, data_B, len(data_A), R)
        if (float(pval) <= float(alpha)):
            print("\nTest result is significant with p-value: {}".format(pval))
            return
        else:
    ...
    if(name=="Bootstrap"):
        ...
        pval = Bootstrap_torch(data_A, data_B, len(data_A), R)
        if (float(pval) <= float(alpha)):
            print("\nTest result is significant with p-value: {}".format(pval))
            return
        else:
