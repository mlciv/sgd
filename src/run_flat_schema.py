...
from modules.schema_catslot_full_processor import SchemaCatSlotFullProcessor
...
MODEL_CLASSES = {
    ...
    "flat_cat_slots_full_state_bert_snt_pair_match": (FlatCatSlotsFullStateBERTSntPairMatchModel, SchemaCatSlotFullProcessor),
    "flat_cat_slots_bert_cls_value": (FlatCatSlotsBERTCLSValueMatchModel, SchemaCatSlotProcessor),
    "flat_cat_slots_bert_snt_pair_match_utt2": (FlatCatSlotsBERTSntPairMatchModel, SchemaCatSlotProcessor),
...
def train(args, config, train_dataset, model, processor):
    ...
    logger.info("parameter names: {}".format([n for n, p in model.named_parameters()]))
    if "toptrans" in args.model_type:
        # here we use large learning rate for the top layers
        optimizer_grouped_parameters = [
            {
                "params": [p for n, p in model.named_parameters() if not any(nd in n for nd in no_decay) and "_matching_layer" in n],
                "weight_decay": args.weight_decay,
                "lr": args.learning_rate * 10,
            },
            {
                "params": [p for n, p in model.named_parameters() if not any(nd in n for nd in no_decay) and "_matching_layer" not in n],
                "weight_decay": args.weight_decay
            },
            {"params": [p for n, p in model.named_parameters() if any(nd in n for nd in no_decay)], "weight_decay": 0.0},
        ]
    else:
        # Here, is diveding the paramaters into weight decay and or not.
        optimizer_grouped_parameters = [
            {
    ...
    if args.model_type not in NO_EMBEDDING_CLASSES:
...
#                    )
            ...
            # loss = 2*losses["loss_cat_slot_status"]/epoch + losses["loss_cat_slot_value"]
...
def evaluate(args, config, model, processor, mode, step="", tb_writer=None):
    ...
    for batch in tqdm(eval_dataloader, desc="Evaluating", position=0, leave=True):
        ...
        with torch.no_grad():
            ...
            # old_predictions = model.define_predictions(inputs, outputs[0])
            # predictions = model.define_oracle_predictions(inputs, labels)
            # logger.info("pred :noncat_slot_status: {}, noncat_slot_start:{}, noncat_slot_end:{}".format(old_predictions["noncat_slot_status"], old_predictions["noncat_slot_start"], old_predictions["noncat_slot_end"]))
            # logger.info("oracle :noncat_slot_status: {}, noncat_slot_start:{}, noncat_slot_end:{}".format(predictions["noncat_slot_status"], predictions["noncat_slot_start"], predictions["noncat_slot_end"]))
            # for flat examples, we need to use the schema ids
            if all_examples_types[0] == 0:
                for i in range(len(all_examples_ids)):
                    # when it is the dst example, every example is a whole predicion
                    prediction = {}
                ...
                for i in range(len(all_examples_ids)):
                    local_prediction = {}
                    for key in predictions.keys():
                        local_prediction[key] = predictions[key][i]
                    _, dialog_id, turn_id, service_name = [x.rstrip() for x in bytes(local_prediction["example_id"].tolist()).decode("utf-8").split("-")]
                    turn_idx = int(turn_id)
                    dial_key = (dialog_id, turn_idx, service_name)
                    prediction = all_predictions.get(dial_key, {})
                    if len(prediction) == 0:
                        prediction["example_id"] = local_prediction["example_id"]
                        prediction["service_id"] = local_prediction["service_id"]
                    # initial unpredicted slot and value as -1, invalid sigmoid value, for padding intent
                    intent_status = prediction.get("intent_status", [-1] * max_intent_num)
                    intent_status[local_prediction["intent_id"]] = local_prediction["intent_status"]
                    if "intent_status" not in prediction:
                        prediction["intent_status"] = intent_status
                ...
                for i in range(len(all_examples_ids)):
                    ...
                    if len(prediction) == 0:
                        prediction["example_id"] = local_prediction["example_id"]
                        prediction["service_id"] = local_prediction["service_id"]
                    # initial unpredicted slot and value as -1, invalid sigmoid value for padding reqslot
                    req_slot_status = prediction.get("req_slot_status", [-1] * max_req_slot_num)
                    req_slot_status[local_prediction["req_slot_id"]] = local_prediction["req_slot_status"]
                    if "req_slot_status" not in prediction:
                        prediction["req_slot_status"] = req_slot_status
                ...
                for i in range(len(all_examples_ids)):
                    ...
                    # logger.info("dial_key:{}, local_prediction:{}".format(dial_key, local_prediction))
                    if "cat_slot_value_status" in local_prediction:
                        ...
                        if "cat_slot_value_status" not in prediction:
                            prediction["cat_slot_value_status"] = cat_slot_value_status

                    if "cat_slot_status" in local_prediction:
                        # initial unpredicted slot and value as -1, which will be ignored when prediction
                        cat_slot_status = prediction.get(
                            "cat_slot_status", [schema_constants.STATUS_OFF] * max_cat_slot_num)
                    ...
                    if "cat_slot_value_full_state" in local_prediction:
                        cat_slot_value = prediction.get("cat_slot_value_full_state", [0] * max_cat_slot_num)
                        cat_slot_value[local_prediction["cat_slot_id"]] = local_prediction["cat_slot_value_full_state"]
                        if "cat_slot_value_full_state" not in prediction:
                            prediction["cat_slot_value_full_state"] = cat_slot_value
                ...
                for i in range(len(all_examples_ids)):
                    ...
                    if len(prediction) == 0:
                        prediction["example_id"] = local_prediction["example_id"]
                        prediction["service_id"] = local_prediction["service_id"]
                        prediction["noncat_alignment_start"] = local_prediction["noncat_alignment_start"]
                        prediction["noncat_alignment_end"] = local_prediction["noncat_alignment_end"]
                    # initial unpredicted slot and value as -1, invalid sigmoid value
                    noncat_slot_status = prediction.get(
                        "noncat_slot_status", [-1] * max_noncat_slot_num)
                    noncat_slot_status[local_prediction["noncat_slot_id"]] = local_prediction["noncat_slot_status"]
                    if "noncat_slot_status" not in prediction:
                        prediction["noncat_slot_status"] = noncat_slot_status
                    # initial start end as -1
                    noncat_slot_start = prediction.get(
                        "noncat_slot_start", [-1] * max_noncat_slot_num)
                    noncat_slot_start[local_prediction["noncat_slot_id"]] = local_prediction["noncat_slot_start"]
                    if "noncat_slot_start" not in prediction:
                        prediction["noncat_slot_start"] = noncat_slot_start
                    noncat_slot_end = prediction.get(
                        "noncat_slot_end", [-1] * max_noncat_slot_num)
                    noncat_slot_end[local_prediction["noncat_slot_id"]] = local_prediction["noncat_slot_end"]
                    if "noncat_slot_end" not in prediction:
                        prediction["noncat_slot_end"] = noncat_slot_end
                    if dial_key not in all_predictions:
                        all_predictions[dial_key] = prediction

            else:
                raise NotImplementedError("{} example type is not supported yet".format(all_examples_types[0]))
    ...
    if evaluate_utils.SEEN_SERVICES in all_metric_aggregate:
        logger.info("Model_step: {}, SEEN_SERVICES Dialog metrics: {}".format(
            step,
            str(all_metric_aggregate[evaluate_utils.SEEN_SERVICES])))

    if evaluate_utils.UNSEEN_SERVICES in all_metric_aggregate:
        logger.info("Model_step: {}, UNSEEN_SERVICES Dialog metrics: {}".format(
            step,
...
def main():
    ...
    EncoderUtils.add_special_tokens(tokenizer, None, schema_constants.USER_AGENT_SPECIAL_TOKENS)
    ...
    processor = processor_class(
        ...
        tokenizer,
        max_seq_length=args.max_seq_length,
        max_schema_seq_length=config.schema_max_seq_length,
        dialog_cxt_length=config.dialog_cxt_length,
