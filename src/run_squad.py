...
def load_and_cache_examples(args, tokenizer, evaluate=False, output_examples=False):
    ...
    # train_file , predict_file is just the split
    split = args.predict_file if evaluate else args.train_file
    # all the files for dialog
    dialog_paths = [
        os.path.join(input_dir, split,
                     "dialogues_{:03d}.json".format(i))
        for i in config.DATASET_CONFIG[args.task_name].file_ranges[split]
    ]

    # the schema file for current split
    input_file_schema = os.path.join(input_dir, split, "schema.json")
    ...
    if os.path.exists(cached_features_file) and not args.overwrite_cache:
        ...
        # TODO: here, how to handle the schema loading
...
def main():
    ...
    parser.add_argument(
        ...
        default=None,
        type=str,
        choices=config.DATASET_CONFIG.keys(),
        required=True,
    ...
    parser.add_argument(
        "--dialogue_example_dir",
        default=None,
        type=str,
        required=True,
        help="Directory where dialogue examples are stored in."
    )
